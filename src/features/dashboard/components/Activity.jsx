...
const Activity = ({ item }) => {
  ...
  return (
    <div className="d-flex align-items-start mb-3">
      {isStock ? (
        <div className="icons btn btn-outline-warning rounded-circle d-flex align-items-center mt-3">
          <BsClipboard2CheckFill color="#ffc107" />
        </div>
      ) : (
        <div className="icons btn btn-outline-info rounded-circle d-flex align-items-center  mt-3">
      ...
      <div className="ms-3 mt-1">
        <h6 className="text-muted fw-medium mb-1">
          {isStock ? "Nouvel ajout de stock !" : "Nouvelle vente enregistrée !"}
        </h6>

        <p className="text-muted mb-1">
          {item.name} {isStock ? "a ajouté" : "vient d'acheter"}
